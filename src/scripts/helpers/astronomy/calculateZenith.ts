...
const calculateLST = (latitude: number, longitude: number, date: Dayjs): number => {
...
// Fonction pour calculer le zénith (RA et DEC)
const calculateZenith = (latitude: number, longitude: number, date: Dayjs): { ra: number, dec: number } => {
    // Calculer le temps sidéral local (LST) en degrés
    const lst = calculateLST(latitude, longitude, date);

    // Le RA du zénith est égal au LST
    const ra = lst;

    // La DEC du zénith est égale à la latitude géographique
    const dec = latitude;

    return { ra, dec }; // Retourner les coordonnées du zénith
};